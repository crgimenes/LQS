...
func (l *Lua) mapToLuaTable(m map[string]interface{}) *lua.LTable {
...
func (l *Lua) SetGlobal(name string, value any) {
	...
	case []any:
		luaTable := l.ls.CreateTable(len(v), 0)
		for i, s := range v {
			l.ls.RawSet(luaTable, lua.LNumber(i+1), l.fromGoToLua(s))
		}
		l.ls.SetGlobal(name, luaTable)
		return
	default:
...
func (l *Lua) GetReturnValues() []any {
	var ret []any
	for i := 1; i <= l.ls.GetTop(); i++ {
		v, err := fromLuaToGo(l.ls.Get(i))
		if err != nil {
			log.Fatalf("Erro ao converter %q para any", i)
		}
		ret = append(ret, v)
	}
	return ret
